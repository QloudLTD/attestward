...
| `scanner-signatures.yaml` | SAST/SCA/container/secrets/SBOM/provenance tool detection signatures | issue #16 |
...
fixture workflow instead). `self-attestation-questions.yaml` is a third case: its
`ssdf_tasks` references must resolve to real ssdf-800-218.yaml task IDs (enforced by the
loader), but the question wording itself is originally authored, not a verbatim
citation — see that file's own header comment.
