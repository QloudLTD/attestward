...
// unknown fields, duplicate task IDs, malformed ID formats, and
...
// docs/schema/mapping-ssdf-800-218.schema.json.
func LoadSSDF(path string) (*SSDFMapping, error) {
	...
	for _, task := range m.Tasks {
		if !taskIDPattern.MatchString(task.ID) {
			return nil, fmt.Errorf("%s: task id %q does not match the SSDF task ID format (e.g. PO.5.1)", path, task.ID)
		}
		if !practiceIDPattern.MatchString(task.Practice) {
			return nil, fmt.Errorf("%s: task %s: practice %q does not match the SSDF practice ID format (e.g. PO.5)", path, task.ID, task.Practice)
		}
		if !strings.HasPrefix(task.ID, task.Practice+".") {
			return nil, fmt.Errorf("%s: task %s: practice %q is not a prefix of its own task id", path, task.ID, task.Practice)
		}
		if task.Family != strings.SplitN(task.ID, ".", 2)[0] {
			return nil, fmt.Errorf("%s: task %s: family %q does not match the task id's family prefix", path, task.ID, task.Family)
		}
		if _, exists := m.Practices[task.Practice]; !exists {
			return nil, fmt.Errorf("%s: task %s: practice %q has no entry in practices", path, task.ID, task.Practice)
		}
		if _, dup := m.TaskByID[task.ID]; dup {
			return nil, fmt.Errorf("%s: duplicate task id %q", path, task.ID)
		}
