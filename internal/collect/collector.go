...
type Scope struct {
	...
	// rather than inferred from GHESVersion.
	//
	// The distinction matters and is not pedantic: GHESVersion is empty
	// both for github.com and for a GHES install whose version header
	// never reached us — a proxy stripping unknown X-* headers, or an
	// error generated by a load balancer ahead of GHES. A collector that
	// inferred "github.com" from an empty version would write a
	// github.com-specific reason (naming a plan tier that does not exist
	// on a self-hosted install) into a pack whose own scope.github_url
	// names that install. This field is what the configured target
	// actually was; GHESVersion is what we happened to learn.
	IsGHES bool
}
