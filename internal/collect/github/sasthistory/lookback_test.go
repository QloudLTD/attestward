...
// documents a scoping decision, not a bug to fix: this collector's input
// comes from GitHub's Releases API (issue #17's own "API evidence" list —
// GET /repos/{o}/{r}/releases), which only returns actual GitHub Release
// objects, never raw git tags without one. A tag pushed without ever
// creating a release for it — the issue's own listed edge case — never
// becomes a releaseInfo in the first place, so it's excluded by
// construction rather than by any filtering logic here.
