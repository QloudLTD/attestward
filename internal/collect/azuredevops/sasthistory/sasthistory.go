...
const sharedUpstreamFetchFailureRubric = "the project's repositories or pipelines couldn't be read " +
	...
	"failure, independent of the scanned repo — since issue #255, this fallback is no longer reachable via " +
	"`attestward scan`: the orchestrator's own load of the same embedded file now aborts the whole scan " +
	"first if it fails; kept as defense in depth for any caller that doesn't go through scan.go's own " +
	"pre-load)"
...
var checkRubrics = map[string]map[model.Status]string{
	idToolConfigured: {
		...
		model.StatusNotCheckable: sharedUpstreamFetchFailureRubric + "; or there is no pipeline-based " +
			...
			"403 (most likely the token lacks the vso.advsec scope; licensing is ruled out as the cause — " +
			"observed 2026-07-23 against dev.azure.com/seciq: an unlicensed org/project's enablement endpoint " +
			"reads HTTP 200, not 403 — but other permission causes can't be excluded from the response " +
			"alone) or any other API error — an unresolved unknown, not a confirmed absence; or one or more " +
