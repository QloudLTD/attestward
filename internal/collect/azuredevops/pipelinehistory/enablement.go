...
//     explicitly set"): encoding/json silently leaves a bool field at its
//     zero value (false) when the JSON value is null, which is exactly the
//     fallback every caller of these three fields wants — "never
//     explicitly set" reads the same as "not enabled" for every purpose
//     C05/C06 have, so a pointer's extra nil-vs-false distinction has no
...
// predicate a caller applies to decide whether a given failure means
// "GHAzDO isn't licensed for this org/project" (a real fact) versus a
// genuine unknown — see that predicate's own doc comment for the
// unconfirmed 403-vs-404 split [fixture-verify], the same open item issue
// #34's epic already tracks for every other advsec endpoint.
