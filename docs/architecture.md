...
`X-GitHub-Enterprise-Version` response header on the first authenticated call (GHES
sends it on every response; github.com never does — so its presence, not the configured
URL, is the authoritative "is this GHES" signal) and is threaded into
`collect.Scope.GHESVersion` at preflight, the same "resolve once, carry it" pattern
`Scope.AccountType` already follows. `ghcollect.ClassifyGate`/`GateReason` replace a
single `IsPlanGated`-derived "not available on this org's plan" Reason (still correct on
github.com, unchanged) with `GateKindLicence`/`GateKindVersion` on a GHES target — GHES
has no per-org plan tier at all, so "plan" is simply wrong there. Absent a genuinely
known per-endpoint minimum GHES version, a gate defaults to `GateKindLicence` (GitHub
Advanced Security or an equivalent add-on most likely unlicensed) rather than a
version claim this tool can't back. Wired into C09 audit-logging's org audit-log probe
