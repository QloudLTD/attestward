...
> **These are templates, not a published action.** Attestward is hosted on
> GitLab and does not publish a GitHub Action. `examples/ci/github/action.yml`
> is a working composite action you can copy into your own repository; the
> GitLab CI equivalent lives beside it in `examples/ci/gitlab/`. Both run the
> same thing: a pinned, signature-verified release binary in read-only mode.
> ⚠ Pin `version:` to a release that exists on your chosen host — the binary is
> fetched from the release assets, so the template is only as good as that pin.
