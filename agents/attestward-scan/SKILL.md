...
`module github.com/sioakim/attestward`), **use it** — do not clone a second copy. Say so.

Otherwise ask where to put it, then:

```bash
git clone https://github.com/sioakim/attestward.git
cd attestward
```
...
`go install github.com/sioakim/attestward/cmd/attestward@latest`, or a signed archive from
the [releases page](https://github.com/sioakim/attestward/releases). Mention it if the
