...
// #150 (S4, two PRs), #151 (S5's first PR), #154 (S8, two PRs), and #152
// (S6, its first collector PR), an azuredevops scan gets every real
// collector landed so far — C01 org-security, C02 repo-protection, C03
// env-separation, C09 audit-logging, C10 vdp, and C05 sast-history —
...
// repoprotection, vdp, and sasthistory specifically, since all three are
// the first project/repo-scoped ADO collectors that consult scope.Repos,
// but not one buildScanDeps itself needs to close (repo resolution
// happens later, in runScan; env-separation never consults scope.Repos at
// all). Asserting all six collector IDs (not just a non-zero count)
...
func TestBuildScanDeps_AzureDevOpsWiresOrgSecurityCollector(t *testing.T) {
	...
	if len(deps.collectors) != 6 {
		t.Fatalf("azuredevops scanDeps has %d collectors, want exactly 6 (C01 org-security, C02 repo-protection, C03 env-separation, C09 audit-logging, C10 vdp, C05 sast-history)", len(deps.collectors))
	}
	gotIDs := map[string]bool{}
	for _, c := range deps.collectors {
		gotIDs[c.ID()] = true
	}
	for _, wantID := range []string{"C01.org-security", "C02.repo-protection", "C03.env-separation", "C09.audit-logging", "C10.vdp", "C05.sast-history"} {
		if !gotIDs[wantID] {
