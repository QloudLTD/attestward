...
import (
	...
	adoauditlogging "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/auditlogging"
	adoenvseparation "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/envseparation"
	adoorgsecurity "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/orgsecurity"
	adopipelinesecurity "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/pipelinesecurity"
	adoprovenance "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/provenance"
	adorepoprotection "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/repoprotection"
	adosasthistory "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/sasthistory"
	adoscahistory "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/scahistory"
	adosecretshygiene "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/secretshygiene"
	advdp "gitlab.com/sioakeim/attestward/internal/collect/azuredevops/vdp"
	ghcollect "gitlab.com/sioakeim/attestward/internal/collect/github"
	"gitlab.com/sioakeim/attestward/internal/collect/github/actionssecurity"
	"gitlab.com/sioakeim/attestward/internal/collect/github/auditlogging"
	"gitlab.com/sioakeim/attestward/internal/collect/github/envseparation"
	"gitlab.com/sioakeim/attestward/internal/collect/github/orgsecurity"
	"gitlab.com/sioakeim/attestward/internal/collect/github/provenance"
	"gitlab.com/sioakeim/attestward/internal/collect/github/repoprotection"
	"gitlab.com/sioakeim/attestward/internal/collect/github/sasthistory"
	...
	gogsvdp "gitlab.com/sioakeim/attestward/internal/collect/gogs/vdp"
...
// defaultGogsCollectors mirrors defaultGitHubCollectors' role for a
// --platform gogs scan (Gogs issues #3, #4).
//
// Two collector sources, and the split is the whole shape of this
// platform. gogsvdp is a real collector making real API calls. The
// unsupported package supplies one collector per remaining check family,
// each emitting not-checkable results for controls Gogs either has no
// mechanism for or exposes data this build does not yet read — see that
// package's doc comment. Both are wired unconditionally, because a Gogs
// pack that quietly contained only the answerable checks would read as a
// clean scan rather than an honest one.
//
// A construction failure is swallowed here rather than propagated: the
// only way NewClient fails is an unusable base URL, which
// scanConfig.validate has already rejected before this is ever reached. If
// it somehow did fail, the vdp collector would be absent while the
...
func defaultGogsCollectors(baseURL, token string) []collect.Collector {
	...
	return append(collectors,
		gogsvdp.New(baseURL, token),
		gogsrepoprotection.New(baseURL, token),
	)
}
...
// an existing az-CLI environment already has it set), GOGS_TOKEN for gogs.
// GITHUB_TOKEN is never consulted for a non-github scan, and each error says
// so explicitly rather than leaving a user with GITHUB_TOKEN exported to
// guess why it didn't help.
func resolveScanToken(platform string) (string, error) {
	switch effectivePlatform(platform) {
	case platformAzureDevOps:
		...
		return pat, nil
	case platformGogs:
		// A Gogs personal access token, from the instance's
		// /user/settings/applications page. It is the only credential
		// its REST API accepts — HTTP basic auth with the account
		// password is rejected with a 401 even when correct — so there
		// is deliberately no username/password fallback here.
...
func buildScanDeps(cfg scanConfig, token string, stdout io.Writer) (scanDeps, error) {
	...
	if effectivePlatform(cfg.Platform) == platformGogs {
		collectors := defaultGogsCollectors(cfg.GogsURL, token)
		if len(collectors) == 0 {
			return scanDeps{}, fmt.Errorf("this build has no collectors for platform %q, so a scan would produce a pack with no verified results at all", platformGogs)
		}
		deps := scanDeps{
			collectors: collectors,
			stdout:     stdout,
		...
		// A repo lister, unlike the azuredevops branch, which still has
		// none: Gogs lists an account's repos from a single endpoint, so
		// `--repo` is a way to narrow a scan here, not a requirement for
		// one to work at all. orgChecker stays nil — Gogs does not
		// distinguish a user account from an organization on any endpoint
		// this scan touches (see gogsRepoLister), so there is no account
		// type to resolve and nothing a preflight check could report that
		// the repo listing does not already prove.
		if client, err := gogscollect.NewClient(cfg.GogsURL, token); err == nil {
			deps.repoLister = &gogsRepoLister{client: client}
		} else {
			// Unreachable in practice — validate() has already rejected
			// an unusable --gogs-url. Left as an explicit branch rather
			// than an ignored error so that if it ever does happen, the
			// scan fails on resolveRepos' nil-lister guard (an actionable
			// "pass --repo" error) instead of panicking.
			logf(stdout, "warning: could not build a Gogs client for repo listing (%v); pass --repo explicitly\n", err)
		}
		return deps, nil
	}
