...
func TestAllC01ThroughC10ChecksHaveRemediation(t *testing.T) {
	...
	// Gogs (issues #4-#6) adds 40 more, for 134. Unlike the Azure DevOps
	// additions above, most are checks the platform cannot evidence at
	// all: internal/collect/gogs/unsupported registers 36 (C01's 4, C03's
	// 4, C04's 5, C05's 4, C06's 5, C07's 5, C08's 5, C09's 4), each
	// deliberately reporting not-checkable with a platform-specific reason
	// rather than being omitted — a pack missing those rows would read as
	// a clean scan rather than an honest one. internal/collect/gogs/vdp
	// registers the remaining 4. C02 repo-protection has no Gogs entry
	// yet; when it lands, this count moves again.
	...
	if len(registered) != 140 {
		t.Fatalf("len(collect.Registered()) = %d, want 140 — did the registered check count change?", len(registered))
	}
}
