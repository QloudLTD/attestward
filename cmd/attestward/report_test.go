...
func TestRunReport_ByteIdenticalToDirectRenderersCall(t *testing.T) {
	...
	// Built through the SAME per-platform resolution production uses, for
	// the reason the scopeLevel comment below gives — and this one is not
	// hypothetical. Until Gogs landed, this map was built by flat-ranging
	// Registered(), which is last-wins per check ID. Registered() sorts by
	// platform then ID, so with only azuredevops and github registered, the
	// GitHub entry won and the flat range happened to agree with production
	// on this GitHub fixture pack. Adding a third platform whose name sorts
	// after "github" made it lose: every finding in this fixture rendered
	// with the *Gogs* remediation text ("This control cannot be enforced or
	// evidenced on Gogs...") while production correctly rendered GitHub's.
	// The bug was in this test, not in buildRemediationByCheckID — which
	// has resolved per-platform since review of #164 for exactly this
	// reason — but it had been silently unable to detect the very
...
func TestReportGo_NoNetworkImports(t *testing.T) {
	...
	forbidden := map[string]bool{
		"net":      true,
		"net/http": true,
		"gitlab.com/sioakeim/attestward/internal/collect/github": true,
	}
